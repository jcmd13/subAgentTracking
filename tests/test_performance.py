...
def benchmark_function(func, iterations: int = NUM_BENCHMARK_ITERATIONS, warmup: int = NUM_WARMUP_ITERATIONS) -> Dict[str, float]:
    """
...
class TestLoggingPerformance:
    ...
    def test_logging_throughput(self, perf_system):
        ...
        assert throughput > 1000, \
            f"Throughput too low: {throughput:.0f} events/sec (target: >1000)"
...
class TestSnapshotPerformance:
    ...
    def test_snapshot_creation_performance(self, perf_system):
        ...
        for i in range(10):
            activity_logger.log_agent_invocation(
                agent=f"agent-{i}",
                invoked_by="orchestrator",
                reason="Setup for snapshot test"
            )
        ...
        assert results['mean'] < TARGET_SNAPSHOT_MS, \
    ...
    def test_snapshot_restoration_performance(self, perf_system):
        ...
        assert results['mean'] < 50.0, \
            f"Snapshot restoration too slow: {results['mean']:.3f}ms"
    ...
    def test_snapshot_with_compression_performance(self, perf_system, mock_config):
        ...
        def create_compressed_snapshot():
            snapshot_manager.take_snapshot(
                trigger="compression_test",
                context={"test": "compression"}
            )
...
class TestAnalyticsPerformance:
    ...
    def test_query_agent_performance_benchmark(self, perf_system):
        ...
        assert results['mean'] < TARGET_ANALYTICS_SIMPLE_MS, \
    ...
    def test_filtered_query_performance(self, perf_system):
        ...
        results = benchmark_function(query)
        print_benchmark_results("query_agent_performance(filtered)", results, TARGET_ANALYTICS_SIMPLE_MS)
    ...
    def test_session_summary_performance(self, perf_system):
        ...
        assert results['mean'] < TARGET_ANALYTICS_COMPLEX_MS, \
...
class TestSystemOverhead:
    ...
    def test_batch_operations_overhead(self, perf_system):
        ...
        assert per_event_time < TARGET_LOGGING_MS, \
...
class TestResourceUsage:
    ...
    def test_file_handle_cleanup(self, perf_system):
        ...
        baseline_handles = process.num_fds() if hasattr(process, 'num_fds') else len(process.open_files())
        ...
        after_handles = process.num_fds() if hasattr(process, 'num_fds') else len(process.open_files())
